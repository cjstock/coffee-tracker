const express = require("express");

// recordRoutes is an instance of the express router.
// We use it to define our routes.
// The router will be added as a middleware and will take control of requests starting with path /record.
const recordRoutes = express.Router();

// This will help us connect to the database
const dbo = require("../db/conn");

// This help convert the id from string to ObjectId for the _id.
const ObjectId = require("mongodb").ObjectId;


// This section will help you get a list of all the records.
recordRoutes.route("/beans").get(function (req, res) {
  let db_connect = dbo.getDb("beans");
  db_connect
    .collection("beans")
    .find({})
    .toArray(function (err, result) {
      if (err) {
        res.status(400).send("Error fetching coffees!")
      } else {
        res.json(result);
      }
    });
});

// This section will help you get a single record by id
recordRoutes.route("/beans/:id").get(function (req, res) {
  let db_connect = dbo.getDb();
  let myquery = { _id: req.body.id };
  db_connect
    .collection("beans")
    .findOne(myquery, function (err, result) {
      if (err) {
        res.status(400).send(`Error fetching bean with id ${myquery.id}`)
      } else {
        res.json(result);
      }
    });
});

// This section will help you create a new record.
recordRoutes.route("/beans/add").post(function (req, res) {
  let db_connect = dbo.getDb();
  let myobj = {
    id: req.body.id,
    country: req.body.country,
    region: req.body.region,
    process: req.body.process,
    variety: req.body.variety,
    altitude: req.body.altitude,
    roast: req.body.roast,
    cuppingNotes: req.body.cuppingNotes,
    brewMethods: req.body.brewMethods,
    extraNotes: req.body.extraNotes
  };
  db_connect.collection("beans").insertOne(myobj, function (err, result) {
    if (err) {
      res.status(400).send("Error inserting bean!")
    } else {
      console.log(`Added a new bean with id ${myobj.id}`)
      res.status(204).send();
    }
  });
});

// This section will help you update a record by id.
recordRoutes.route("beans/update/:id").post(function (req, response) {
  let db_connect = dbo.getDb();
  let myquery = { _id: req.body.id };
  let newvalues = {
    $inc: {
      id: req.body.id,
      country: req.body.country,
      region: req.body.region,
      process: req.body.process,
      variety: req.body.variety,
      altitude: req.body.altitude,
      roast: req.body.roast,
      cuppingNotes: req.body.cuppingNotes,
      brewMethods: req.body.brewMethods,
      extraNotes: req.body.extraNotes
    },
  }

  db_connect.collection("beans").updateOne(myquery, newvalues, (err, result) => {
    if (err) {
      response.status(400).send(`Error updating info on bean with id ${myquery.id}`)
    } else {
      console.log("1 document updated");
      response.status(204).send(`Updated bean with id ${myquery.id}`)
    }
  })
});

// This section will help you delete a record
recordRoutes.route("beans/:id").delete((req, response) => {
  let db_connect = dbo.getDb();
  let myquery = { _id: req.body.id };
  db_connect.collection("beans").deleteOne(myquery, function (err, obj) {
    if (err) {
      response.status(400).send(`Error deleting bean with id ${myquery.id}`)
    }
    console.log("1 document deleted");
    response.json(obj);
  });
});

module.exports = recordRoutes;